...
const WAF_CHECKS: {
...
}[] = [
  {
    ...
    description:
      'Checks for audit log streaming configuration events, indicating real-time event forwarding is active.',
    lookback: '30 days',
  },
  {
    id: 'waf-branch-protection',
    pillar: 'governance',
    name: 'Branch protection coverage',
    description:
      'Compares branch protection creations vs. removals/overrides to detect weakening of code review requirements.',
    lookback: '90 days',
  },
  {
    id: 'waf-sso-status',
    ...
    description:
      'Monitors for SSO disable events and tracks whether SAML/SSO is actively configured across organizations.',
    lookback: '90 days',
  },
  {
    id: 'waf-ip-allowlist',
    pillar: 'governance',
    name: 'IP allowlist configuration',
    description:
      'Detects IP allowlist management events to verify network-level access restrictions are in place.',
    lookback: '90 days',
  },
  {
    id: 'waf-webhook-health',
    pillar: 'governance',
    name: 'Webhook lifecycle management',
    description:
      'Tracks webhook creation vs. destruction to identify integration instability or orphaned hooks.',
    lookback: '90 days',
  },
  {
    id: 'waf-push-protection-bypass',
    pillar: 'governance',
    name: 'Push protection bypasses',
    description:
      'Counts instances where developers bypassed secret scanning push protection, overriding security controls.',
    lookback: '90 days',
  },
  {
    id: 'waf-workflow-permissions',
    pillar: 'governance',
    name: 'Workflow permissions changes',
    description:
      'Detects changes to default workflow permissions that may loosen security controls.',
    lookback: '90 days',
  },
  {
    id: 'waf-self-approve-pr',
    pillar: 'governance',
    name: 'Workflow self-approval of PRs',
    description:
      'Monitors for changes to workflow PR self-approval settings that could weaken code review enforcement.',
    lookback: '90 days',
  },
  {
    id: 'waf-deploy-key-policy',
    pillar: 'governance',
    name: 'Deploy key policy status',
    description:
      'Checks for deploy key policy disable events that may allow uncontrolled repository access.',
    lookback: '90 days',
  },
  {
    ...
    description:
      'Tracks environment protection rule additions to verify deployment approval gates are in place.',
    lookback: '90 days',
  },
  {
    ...
    description:
      'Detects admin promotions that should be rare and approved through proper channels.',
    lookback: '90 days',
  },
  {
    id: 'waf-secret-scanning',
    pillar: 'appsec',
    name: 'Secret scanning enablement',
    description:
      'Tracks repositories enabling or disabling secret scanning to identify coverage gaps.',
    lookback: '90 days',
  },
  {
    id: 'waf-dependabot',
    pillar: 'appsec',
    name: 'Dependabot alert coverage',
    description:
      'Monitors Dependabot alert enablement/disablement across repositories for supply chain security.',
    lookback: '90 days',
  },
  {
    id: 'waf-code-scanning',
    pillar: 'appsec',
    name: 'Code scanning activity',
    description:
      'Checks for CodeQL or third-party SAST tool activity to verify static analysis is running.',
    lookback: '90 days',
  },
  {
    id: 'waf-direct-push',
    pillar: 'appsec',
    name: 'Direct pushes to default branch',
    description:
      'Counts direct pushes to main/master branches that bypass the pull request review workflow.',
    lookback: '90 days',
  },
  {
    id: 'waf-actions-secrets',
    pillar: 'appsec',
    name: 'Actions secrets created',
    description:
      'Reports on Actions secret creation events to track credential management activity.',
    lookback: '90 days',
  },
  {
    id: 'waf-vuln-alert-dismissed',
    pillar: 'appsec',
    name: 'Vulnerability alerts dismissed',
    description:
      'Detects dismissed or withdrawn vulnerability alerts that may represent unaddressed security risks.',
    lookback: '90 days',
  },
  {
    id: 'waf-clone-anomaly',
    pillar: 'appsec',
    name: 'Clone activity anomaly detection',
    description:
      'Identifies actors with clone counts exceeding 3× the average, which may indicate data exfiltration.',
    lookback: '30 days',
  },
  {
    id: 'waf-pr-merge-ratio',
    pillar: 'collaboration',
    name: 'Pull request merge ratio',
    description:
      'Compares PR merge count to creation count to detect anomalies in the review workflow.',
    lookback: '90 days',
  },
  {
    ...
    description:
      'Measures the percentage of failed workflow runs to identify CI/CD reliability issues.',
    lookback: '30 days',
  },
  {
    id: 'waf-workflow-rerun-rate',
    pillar: 'productivity',
    name: 'Workflow rerun rate',
    description:
      'Tracks the ratio of workflow reruns to total runs, indicating flaky or unreliable workflows.',
    lookback: '30 days',
  },
];
...
function WafChecksCatalog() {
  ...
  const byPillar = PILLAR_ORDER.reduce<Record<WafPillar, typeof WAF_CHECKS>>(
    (acc, p) => {
  ...
  return (
    <div className={styles.catalogWrapper}>
      <button className={styles.catalogToggle} onClick={() => setOpen(!open)} aria-expanded={open}>
        <svg
          width="12"
          height="12"
          viewBox="0 0 12 12"
          ...
          {PILLAR_ORDER.map((pillar) => {
            ...
            return (
              <div key={pillar} className={styles.catalogPillar}>
                <div className={styles.catalogPillarLabel}>
  ...
  return (
    <>
      ...
      <div className={styles.wafHeader}>
        <div
          className={`${styles.dataSourceNote} ${styles.wafHeaderNote}`}
        ...
        >
          ...
          <span>
            Signals are aligned to the <strong>GitHub Well-Architected Framework</strong> (5
            pillars: Governance, Application Security, Architecture, Collaboration, Productivity).
            Coverage is partial — OctoWatch evaluates signals detectable from audit log events.
          </span>
        </div>
        <a
        ...
        {PILLAR_ORDER.map((pillar) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={styles.pillarCount}
                style={{
                  color: summary.total > 0 ? PILLAR_COUNT_COLOR[pillar] : 'var(--fg-muted)',
                }}
              >
                {summary.total}
              </div>
        ...
        return (
          <div key={pillar} id={`pillar-${pillar}`}>
            <div className={styles.pillarSectionTitle}>
              ...
              <a href={meta.url} target="_blank" rel="noopener" className={styles.wafRef}>
                View pillar ↗
              </a>
            </div>
            ...
            {findings.length === 0 && (
              <div className={styles.productivityOk}>
                <strong style={{ color: 'var(--success)' }}>
                  ✓ No {meta.label.toLowerCase()} anti-patterns detected
                </strong>
              </div>
            )}
